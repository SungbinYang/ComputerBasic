...
컴퓨터가 국가라고 보았을 때 국가의 법은 Kernel로 구현된다고 비유적으로 볼 수 있다. 즉, Kernel영역과 사용자 영역은 완전히 다른 영역이며, 비유적으로 인간계와 신계로 비유할 수 있다. Kernel은 수많은 역할을 하는데 그중에 큰 게 첫째로 I/O 제어를 한다. 둘째로 자원관리를 한다. 여기서 자원이라고 하면 CPU나 메모리를 의미한다. 즉, CPU와 메모리를 어떤 프로세스가 어떤 스레드가 얼마나 쓸 것인지 통제한다. 셋째로 접근통제를 한다.

커널은 일종의 계층이지만 일종의 mode라고 한다. 일반적으로 S/W가 작동할때는 User mode(Application 영역)에서 작동하며 Kernel 모드에서 작동한다는 것은 OS의 일부라고 보면 된다. 물론 OS일부가 아닌 애들도 있는데 그런 것은 바로 보안 쪽으로 보면 된다. 즉, 인터넷뱅킹 같은 것을 할 때 설치하는 프로그램들을 보면 될 것이다.
...
또한 user mode에서 interface가 보조기억장치를 볼 수 있는 프로세스가 존재하는데 이것을 윈도에서는 탐색기, 맥에서는 파인더라고 부른다. 즉, 다시 말해 File System의 파일구조들을 시각화하여 GUI형태로 보여주는 것이다.
...
추가적으로, physical인 하드웨어를 소프트웨어화하는 기술이 있는데 이것을 위에서 언급한 소프트웨어 다른 말인 Virtual로 바꾸면 Virtualization(가상화)가 된다. 이 용어도 잘 알아두자.
